...
namespace DM
{
    ...
    {
        //変更をチェックするディレクトリのパス
        private static readonly string TARGET_DIRECTORY_PATH = "Assets/AssetBundle";
        ...
        //入力されたassetsのパスの中に、親ディレクトリの名前がtargetDirectoryNameListのものが一つでもあるか
        private static bool ExistsDirectoryInAssets(List<string[]> assetPathsList, List<string> targetDirectoryNameList)
        {
            return assetPathsList
                .Any(assetPaths => assetPaths.Any(x => x.Contains(TARGET_DIRECTORY_PATH)));
        }
        ...
        private static void Create()
        {
            ...
            ConstantsClassCreator.Create("AddressableAssetAddress", "AddressableAssetのAddressを管理する定数クラス",
                EXPORT_DIRECTORY_PATH, addressDict);
            ConstantsClassCreator.Create("AddressableAssetLabel", "AddressableAssetのLabelを管理する定数クラス",
